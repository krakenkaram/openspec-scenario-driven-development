...
describe("openSchemaFile handler (wiring)", () => {
  it("delegates to core with the scan args, schema name, repo, and injected reveal opener", async () => {
    ...
    expect(openSchemaFile).toHaveBeenCalledWith({ repos: [], root: "/x", depth: 1 }, "atdd-driven", "/repo", reveal);
  });
